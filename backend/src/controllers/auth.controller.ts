    ...
    FileInterceptor('avatar', {
      ...
      fileFilter: (req, file, callback) => {
        if (!file.mimetype.match(/^image\/(jpg|jpeg|png|gif)$/)) {
          return callback(new Error('Chỉ chấp nhận file ảnh JPG, JPEG, PNG, GIF'), false);
        }
        callback(null, true);
      },
    }),
  )
  async register(@UploadedFile() file: Express.Multer.File, @Body() body: RegisterDto) {
    const avatarPath = file ? file.path : 'uploads/avatars/default.png'; // 🖼 Ảnh mặc định nếu không upload
